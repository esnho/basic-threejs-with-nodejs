...
//  var bordo1 = getBox(1, 0.1, 0.1, 0);
  ...
  var clearColor = fogColor// new THREE.Color(); //fogColor
  ...
  update(renderer, scene, camera, controls, velocity, clock);
...
function update(renderer, scene, camera, camControls, velocity, clock) {
  velocity.value;
  ...
  boxgrid.children.forEach(function(child, index) {
    child.scale.y = Math.abs(
      Math.sin(
        (timeElapsed * velocity.value) + index
      ) *
      Math.sin(
        ((timeElapsed * velocity.value) + index) * 1.01
      )
    ) + 0.01;
...
/*
  ...
  requestAnimationFrame(function() {
    update(renderer, scene, camera, camControls, velocity, clock);
  });
}
